...
(** Syntactic type *)
Inductive nsynty :=
| aprvarₛ | prvarₛ (X : nsynty) | nsyntyₛ
| Empty_setₛ | unitₛ | boolₛ | natₛ | Zₛ | Propₛ
| optionₛ (X : nsynty) | listₛ (X : nsynty)
| prod'ₛ (X Y : nsynty) | sumₛ (X Y : nsynty) | funₛ (X Y : nsynty).
Implicit Type (X Y : nsynty).

Module NsyntyNotation.
  Notation "()ₛ" := unitₛ.
  Infix "*'ₛ" := prod'ₛ (at level 80, right associativity).
  Infix "+ₛ" := sumₛ (at level 85, right associativity).
  Infix "→ₛ" := funₛ (at level 99, right associativity).
End NsyntyNotation.
...
Fixpoint nsynty_inhab X : bool :=
  match X with
  | prvarₛ X => nsynty_inhab X | Empty_setₛ => false
...
Fixpoint nsynty_ty (X : nsynty) : Type :=
  match X with
  | aprvarₛ => aprvarn | prvarₛ X => prvar X | nsyntyₛ => nsynty
  | Empty_setₛ => Empty_set | unitₛ => () | boolₛ => bool | natₛ => nat
  | Zₛ => Z | Propₛ => Prop
  | optionₛ X => option (nsynty_ty X) | listₛ X => list (nsynty_ty X)
  | X *'ₛ Y => nsynty_ty X *' nsynty_ty Y | X +ₛ Y => nsynty_ty X + nsynty_ty Y
