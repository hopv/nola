...
Section borrow.
  ...
  Proof.
    ...
    { iApply (aconvert_trans with "ΩΦ ΦΨ"). }
  Qed.
  Lemma pobor_alter {X α q ξ Φ Ψ} :
    ⸨ ∀ x, ↑ˡ Ψ x ==∗ ↑ˡ Φ x ⸩(δ) -∗ pobor (X:=X) δ α q ξ Φ -∗ pobor δ α q ξ Ψ.
  Proof.
  ...
  Proof.
    iIntros "⊑ → [%[? o]]". iDestruct (pobor_tok_lft with "⊑ → o") as "o".
    iExists _. iFrame.
  Qed.
...
Section borrow.
  ...
  Local Lemma obor_nobor_tok `{!GenUpd M} {α q P} :
  ...
  Local Lemma pobor_plist {Xl A β}
  ...
  Lemma pobor_merge_subdiv `{!GenUpd M} Xl Yl
    ...
    (∀ yl', [†β] -∗ ([∗ plist] '(y', _, Ψ)' ∈ plist_zip yl' yΨl, ⟦ Ψ y' ⟧) -∗
      ...
      [∗ list] R ∈ Rl, borcd β R.
  Proof.
    rewrite pobor_plist /= -bi.sep_exist_l. setoid_rewrite <-nintpS_nintp.
  ...
  Lemma pobor_subdiv `{!GenUpd M} {X α q ξ Φ} Yl (f : _ → X) (yΨl : plist _ Yl)
    Rl β :
    β ⊑□ α -∗ pobord α q ξ Φ -∗
    ([∗ plist] '(y, Ψ)' ∈ yΨl, ⟦ Ψ y ⟧) -∗ ([∗ list] R ∈ Rl, ⟦ R ⟧) -∗
    (∀ yl', [†β] -∗ ([∗ plist] '(y', _, Ψ)' ∈ plist_zip yl' yΨl, ⟦ Ψ y' ⟧) -∗
  ...
  Proof.
    iIntros "⊑ o Ψl Rl →Φ".
    iMod (pobor_merge_subdiv [_] _ ((_,_,_,_,_)',())'
  ...
  Lemma pobor_resolve `{!GenUpd M} {X α q ξ Φ} (x : X) :
    pobord α q ξ Φ -∗ ⟦ Φ x ⟧ =[pborrow_wsatd M]=∗
      q.[α] ∗ ⟨π, π ξ = x⟩ ∗ borcd α (Φ x).
  Proof.
    iIntros "o Φ".
    iMod (pobor_subdiv [] (λ _, x) () [Φ x] with "[] o [//] [Φ] []")
      as (?) "[$[$[_[$ _]]]]"=>/=;
  ...
  Proof.
    ...
    iMod (pobor_resolve with "o Φ") as "$". by iIntros.
  Qed.
  ...
  Lemma pobor_close `{!GenUpd M} {X α q ξ Φ} (x : X) :
    pobord α q ξ Φ -∗ ⟦ Φ x ⟧ =[pborrow_wsatd M]=∗ q.[α] ∗ pborcd α x ξ Φ.
  Proof.
    iIntros "o Φ". iApply (pobor_nsubdiv Φ with "[] o Φ"); [|by iIntros].
    iApply lft_sincl_refl.
  Qed.

  (** Lemma for [pobor_pobor_reborrow] *)
  Local Lemma pobor_pobor_tok `{!GenUpd M} {Y β r η Ψ} {y : Y} :
    pobord β r η Ψ -∗ ⟦ Ψ y ⟧ =[pborrow_wsatd M]=∗ pobor_tok β r η Ψ ∗ ⟦ Ψ y ⟧ˢ.
  Proof.
  ...
  Qed.
  Lemma pobor_pborc_reborrow `{!GenUpd M} {X Y α q ξ Φ β r y η Ψ} (f : X → Y) :
    pobord α q ξ Φ -∗ r.[β] -∗ pborcd β y η Ψ -∗
    (∀ y', [†α] -∗ pbord β y' η Ψ -∗ M ⟦ Φ (f y') ⟧) =[pborrow_wsatd M]=∗ ∃ η',
      q.[α] ∗ r.[β] ∗ ⟨π, π ξ = f (π (Aprvar _ η'))⟩ ∗ pborcd (α ⊓ β) y η' Ψ.
  Proof.
    iIntros "o r c →Φ". iMod (pborc_open with "r c") as "[o' Ψ]".
    by iMod (pobor_pobor_reborrow with "o o' Ψ →Φ").
  Qed.
  Lemma pobor_pbor_reborrow `{!GenUpd M} {X Y α q ξ Φ β r y η Ψ} (f : X → Y) :
