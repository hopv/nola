...
Section lemmas.
  ...
  Proof.
    rewrite na_nninv_unseal. iIntros (NE NF) "#accP F".
    iDestruct nderiv_sound as "to".
  ...
  Lemma na_ninv_nninv (P : nPropS _) {i p N} : i ∈ (↑N:coPset) →
    ninv N (nInvd_na p i P) -∗ na_nninv (Σ:=Σ) d p N (↑ˡ P).
  Proof.
  ...
  Lemma na_nninv_alloc_rec (P : nPropS _) {p N} :
    (na_nninv d p N (↑ˡ P) -∗ ⟦ P ⟧(d)) =[nninv_wsat d]=∗
      na_nninv d p N (↑ˡ P).
  Proof.
    iIntros "toP". iMod na_lock_alloc as (i) "[% lock]".
    iMod (ninv_alloc_rec (P:=nInvd_na p i P) with "[toP lock]") as "NP".
  ...
  Lemma na_nninv_alloc (P : nPropS _) {p N} :
    ⟦ P ⟧(d) =[nninv_wsat d]=∗ na_nninv d p N (↑ˡ P).
  Proof.
  ...
  Lemma na_nninv_convert {p N P Q} :
    □ ⸨ P ={∅}=∗ Q ∗ (Q ={∅}=∗ P) ⸩(d) -∗ na_nninv d p N P -∗ na_nninv d p N Q.
  Proof.
  ...
  Lemma na_nninv_add {p N P Q} :
    □ ⸨ P ⸩(d) -∗ na_nninv d p N Q -∗ na_nninv d p N (P ∗ Q).
  Proof.
  ...
  Lemma nninv_combine {p N N' N'' P Q} : N ## N' → ↑N ∪ ↑N' ⊆@{coPset} ↑N'' →
    na_nninv d p N P -∗ na_nninv d p N' Q -∗ na_nninv d p N'' (P ∗ Q).
  Proof.
