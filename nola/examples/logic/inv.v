...
Section lemmas.
  ...
  Lemma ninv_nninv {s i N P} : ninv N P -∗ nninv (Σ:=Σ) (σ s) i N (nlarge P).
  Proof.
  ...
  (** Allocate [nninv] *)
  Lemma nninv_alloc_rec {s i N} {P : nPropS _} :
  ...
  Qed.
  Lemma nninv_alloc {s i N} {P : nPropS _} :
  ...
  Proof.
    ...
    iFrame "NR". iModIntro. iApply sintpy_byintp. iIntros (??) "/= #to _ _ R".
    iDestruct ("to" with "PQP") as "/={PQP}PQP".
    iDestruct ("to" with "RPR") as "/={RPR}RPR".
    iMod ("RPR" with "R") as "[P PR]". iMod ("PQP" with "P") as "[$ QP]".
    iIntros "!> Q". iMod ("QP" with "Q") as "P". by iApply "PR".
  Qed.
  ...
  Proof.
    rewrite nninv_unseal. iIntros "#P (%R & #RPR & #NR)". iExists R.
    iFrame "NR". iModIntro. iApply sintpy_byintp. iIntros (??) "/= #to _ _ R".
    iDestruct ("to" with "P") as "/={P}$".
    iDestruct ("to" with "RPR") as "/={RPR}RPR".
    iMod ("RPR" with "R") as "[$ QR]". iIntros "!> [_ Q]". by iApply "QR".
  Qed.
End lemmas.
