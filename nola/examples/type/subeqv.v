...
Section subeqv.
  ...
  (** Basic *)
  Lemma tsub_refl {s i T} : T ⊑{i,i}(s) T.
  Proof. done. Qed.
  Lemma tsub_trans {s i T j U k V} :
    T ⊑{i,j}(s) U → U ⊑{_,k}(s) V → T ⊑(s) V.
  Proof. move=> TU UV ?. by rewrite TU UV. Qed.
  Lemma teqv_refl {s i T} : T ≃{i,i}(s) T.
  Proof. done. Qed.
  Lemma teqv_symm {s i T j U} : T ≃{i,j}(s) U → U ≃{j,i}(s) T.
  ...
  (** On [∧ᵗ] *)
  Lemma tsub_and_elim_l {s i T U} : T ∧ᵗ U ⊑{i,i}(s) T.
  Proof. iIntros (?) "/=[$ _]". Qed.
  Lemma tsub_and_elim_r {s i T U} : T ∧ᵗ U ⊑{i,i}(s) U.
  Proof. iIntros (?) "/=[_ $]". Qed.
  Lemma tsub_and_intro {s j V i T U} :
    V ⊑{j,i}(s) T → V ⊑{j,i}(s) U → V ⊑{j,i}(s) T ∧ᵗ U.
  Proof.
    iIntros (VT VU ?) "#V /=".
  ...
  Lemma tsub_fun `{! j ≤ⁿ i, ! j' ≤ⁿ i', ! j ≤ⁿ j'} {s T U T' U'} :
    T' ⊑{i',i}(s) T →  U ⊑{i,i'}(s) U' →  T →(j) U ⊑{i,i'}(s) T' →(j') U'.
  Proof.
