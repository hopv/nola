...
Section borrow.
  ...
  Proof.
    iIntros (Ψ) "(#⊑ & [β β'] & b) →Ψ".
    iMod (bor_tok_open (M:=bupd_0) with "β b") as "/=[o [%l'[>↦ b']]]".
    iApply twpw_fupdw_nonval; [done|]. wp_read.
    iMod (lft_sincl_live_acc with "⊑ β'") as (?) "[α →β']".
    rewrite sem_ecustom /=.
    iMod (bor_tok_reborrow (M:=bupd_0) with "⊑ α b'") as "(α & →b' & b')".
    iDestruct ("→β'" with "α") as "β'".
    iMod (obor_tok_subdiv (M:=bupd_0) [] with "[] o [] [↦ →b']") as "[β _]"=>/=.
  ...
  Context `{!prophGS nsynty Σ, !proph_agG nsynty Σ, !PborCon nsynty CON,
    !PborSem nsynty JUDG CON Σ}.
  ...
  Proof.
    ...
    rewrite sem_ecustom /=.
    iMod (bor_tok_open (M:=bupd_0) with "α b") as "/=[o >↦]". wp_read.
    iMod (obor_tok_close (M:=bupd_0) with "o [↦]") as "[α b]"=>/=; [done|].
