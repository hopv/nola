...
Definition heap_adequacy Σ `{!heapGpreS Σ} X W s e σ φ :
  (∀ `{!heapGS Σ X W}, ⊢ inv_heap_inv -∗ WP e @ s; ⊤ {{ v, ⌜φ v⌝ }}) →
  (∀ `{!invGS Σ}, ⊢ |={⊤}=> ∃ x, W x) → adequate s e σ (λ v _, φ v).
Proof.
  intros Hwp Winit.
  apply adequate_alt; intros t2 σ2 [n [κs ?]]%erased_steps_nsteps.
  eapply (wp_strong_adequacy Σ _); [|done].
  iIntros (Hinv). iMod Winit as (x) "W".
  iMod (gen_heap_init σ.(heap)) as (?) "[Hh _]".
  iMod (inv_heap_init loc (option val)) as (?) ">Hi".
  iMod (proph_map_init κs σ.(used_proph_id)) as (?) "Hp".
  iMod (mono_nat_own_alloc) as (γ) "[Hsteps _]".
  iDestruct (Hwp (HeapGS _ _ _ _ _ _ _ _ _ _ _) with "Hi") as "Hwp".
  iModIntro.
  iExists (λ σ ns κs nt, (W x ∗ gen_heap_interp σ.(heap) ∗
