...
Definition impl1 {A} : relation (A → Prop) := pointwise_relation A impl.
...
Notation Mono1 A := (Proper (@impl1 A ==> @impl1 A)).

Lemma go_mono1 A p `(monop : Mono1 A p) φ ψ a : (φ →1 ψ) → p φ a → p ψ a.
...
Global Instance Mono1_impl `(Mono1 A p) :
  Proper ((→1) ==> (=) ==> (→0)) p.
Proof. move=> φ ψ /=? a _ <- +. by go_mono1. Qed.

Global Instance Mono1_flip_impl `(Mono1 A p) :
  Proper ((→1) --> (=) ==> flip (→0)) p.
