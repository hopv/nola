...
Implicit Type (SI : sidx) (A : Type).

(** ** [poty]: Partially ordered type *)
#[projections(primitive)]
Structure poty {SI} := Poty {
  poty_car :> ofe;
...
Arguments Poty {_}. Arguments poty_car {_ OT} : rename.
Arguments ole {_ OT} : rename. Arguments ole_preorder {_ OT} : rename.
Arguments equiv_ole {_ OT _ _} : rename.
#[export] Typeclasses Transparent ole.

Section poty.
  Context {SI}.
  Implicit Type OT : poty.
  ...
  (** [⊑] is proper *)
  #[export] Instance ole_proper_ole {OT} :
    Proper ((⊑) --> (⊑) ==> impl) (⊑@{OT}).
...
(** Function *)
Program Canonical funP {SI A} (OTF : A → poty) : poty :=
...
Qed.
Module FunPNotation.
  Notation "A -p> OT" := (@funP _ A (λ _, OT))
    (at level 99, OT at level 200, right associativity).
End FunPNotation.
Import FunPNotation.

(** Non-expansive map *)
Program Canonical funNP {SI} (A : ofe) (OT : poty) :=
...
Section poty.
  ...
  Definition dual_equiv_def {SI} {A : ofe} : relation (dual A) :=
    λ '(Dual a) '(Dual a'), a ≡ a'.
  Definition dual_dist_def {SI} {A : ofe} n : relation (dual A) :=
    λ '(Dual a) '(Dual a'), a ≡{n}≡ a'.
  Program Canonical dualO (A : ofe) :=
    @Ofe _ (dual A) dual_equiv_def dual_dist_def _.
  Next Obligation.
    split=> >; [exact: equiv_dist| |exact: dist_le].
...
Class Mono {SI} {OT OT' : poty} (f : OT -p> OT') : Prop :=
  mono :: Proper ((⊑) ==> (⊑)) f.
Hint Mode Mono + + + ! : typeclass_instances.
Class Anti {SI} {OT OT' : poty} (f : OT -p> OT') : Prop :=
  anti :: Proper ((⊑) --> (⊑)) f.
Hint Mode Anti + + + ! : typeclass_instances.
Class Mono2 {SI} {OT OT' OT'' : poty} (f : OT -p> OT' -p> OT'') : Prop :=
  mono2 :: Proper ((⊑) ==> (⊑) ==> (⊑)) f.
Hint Mode Mono2 + + + + ! : typeclass_instances.
Class AntiMono {SI} {OT OT' OT'' : poty} (f : OT -p> OT' -p> OT'') : Prop :=
  antimono :: Proper ((⊑) --> (⊑) ==> (⊑)) f.
...
Section poty.
  ...
  (** Partial application *)
  #[export] Instance mono2_mono_1 `{!@Mono2 _ OT OT' OT'' f} : Mono f.
  Proof. unfold Mono. solve_proper. Qed.
  #[export] Instance mono2_mono_2 `{!@Mono2 _ OT OT' OT'' f} {o} : Mono (f o).
  Proof. unfold Mono. solve_proper. Qed.
  #[export] Instance antimono_mono_1 `{!@AntiMono _ OT OT' OT'' f} : Anti f.
  Proof. unfold Anti. solve_proper. Qed.
  #[export] Instance antimono_mono_2 `{!@AntiMono _ OT OT' OT'' f} {o} :
    Mono (f o).
  Proof. unfold Mono. solve_proper. Qed.

  (** Flip order *)
  #[export] Instance mono_flip `{!@Mono _ OT OT' f} :
    Proper ((⊑) --> flip (⊑)) f.
  Proof. solve_proper. Qed.
  #[export] Instance anti_flip `{!@Anti _ OT OT' f} :
     Proper ((⊑) ==> flip (⊑)) f.
  Proof. solve_proper. Qed.
  #[export] Instance mono2_flip `{!@Mono2 _ OT OT' OT'' f} :
    Proper ((⊑) --> (⊑) --> flip (⊑)) f.
  Proof. solve_proper. Qed.
  #[export] Instance antimono_flip `{!@AntiMono _ OT OT' OT'' f} :
    Proper ((⊑) ==> (⊑) --> flip (⊑)) f.
  Proof. solve_proper. Qed.

  (** Monotonicity implies properness *)
  #[export] Instance mono_proper `{!@Mono _ OT OT' f} : Proper ((≡) ==> (≡)) f.
  Proof. move=> > /equiv_ole[??]. apply equiv_ole. split; exact: mono. Qed.
  #[export] Instance anti_proper `{!@Anti _ OT OT' f} : Proper ((≡) ==> (≡)) f.
  Proof. move=> > /equiv_ole[??]. apply equiv_ole. split; exact: anti. Qed.
  #[export] Instance mono2_proper `{!@Mono2 _ OT OT' OT'' f} :
    Proper ((≡) ==> (≡) ==> (≡)) f.
  Proof.
    move=> > /equiv_ole[??] ?? /equiv_ole[??]. apply equiv_ole.
    split; exact: mono2.
  Qed.
  #[export] Instance antimono_proper `{!@AntiMono _ OT OT' OT'' f} :
    Proper ((≡) ==> (≡) ==> (≡)) f.
  Proof.
    move=> > /equiv_ole[??] ?? /equiv_ole[??]. apply equiv_ole.
...
Class Otop {SI} (OT : poty) : Type := OTOP {
  otop :: Top OT;
  otop_intro {o : OT} : o ⊑ ⊤;
}.
Hint Mode Otop - ! : typeclass_instances.
Arguments OTOP {_ _} _ _.

Class Obot {SI} (OT : poty) : Type := OBOT {
  obot :: Bottom OT;
  obot_elim {o : OT} : ⊥ ⊑ o;
}.
Hint Mode Obot - ! : typeclass_instances.
Arguments OBOT {_ _} _ _.
...
Class BinMeet {SI} (OT : poty) : Type := BIN_MEET {
  bin_meet :: Meet OT;
...
Arguments BIN_MEET {_ _} _ _ _ _.

Class BinJoin {SI} (OT : poty) : Type := BIN_JOIN {
  bin_join :: Join OT;
...
Arguments BIN_JOIN {_ _} _ _ _ _.
...
Section poty.
  ...
  #[export] Program Instance bin_meet_Prop : BinMeet Prop :=
    BIN_MEET and _ _ _ _.
  Next Obligation. by move=> ??[??]. Qed.
  Next Obligation. by move=> ??[??]. Qed.
  Next Obligation. move=> *. split; auto. Qed.

  #[export] Program Instance bin_join_Prop : BinJoin Prop :=
    BIN_JOIN or _ _ _ _.
...
Class BigMeet {SI} OT : Type := BIG_MEET {
...
Arguments BIG_MEET {_ _} _ _ _.

Class BigJoin {SI} OT : Type := BIG_JOIN {
