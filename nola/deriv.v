...
Record judg := Judg {
  ...
  (** Semantics *)
  judg_sem : dwrap (∀ i, judg_data i → Prop) → ∀ i, judg_data i → Prop;
}.
Add Printing Constructor judg.
Arguments judg_sem {JU} d i J : rename.

(** Derivability predicate *)
Notation deriv_ty JU := (dwrap (∀ i, JU.(judg_data) i → Prop)).
(** Derivability predicate parameterized over a derivability predicate *)
Notation pderiv_ty JU := (deriv_ty JU → deriv_ty JU).

(** Modified [judg_sem] *)
Notation judg_sem' d := (Dwrap (judg_sem d)).

(** Notation for derivability *)
Module DerivNotation.
  Notation "⊢{ d , i }" := (dunwrap d i) (format "⊢{ d ,  i }") : nola_scope.
  Notation "⊨{ d , i }" := (judg_sem d i) (format "⊨{ d ,  i }")
    : nola_scope.
  Notation "⟦ d ⟧ᵈ" := (judg_sem' d) (format "⟦ d ⟧ᵈ") : nola_scope.
End DerivNotation.
Import DerivNotation.

(** Operations on derivability predicates *)

Definition Falseᵈ {JU} : deriv_ty JU := Dwrap (λ _ _, False).
Notation "⊥ᵈ" := Falseᵈ : nola_scope.

Definition orᵈ {JU} (d d' : deriv_ty JU) : deriv_ty JU :=
  Dwrap (λ i J, ⊢{d,i} J ∨ ⊢{d',i} J).
...
Definition implᵈ {JU} (d d' : deriv_ty JU) : Prop :=
  ∀ i J, ⊢{d,i} J → ⊢{d',i} J.
...
Record derivy_gen (JU : judg) (self : pderiv_ty JU → Prop) (δ : pderiv_ty JU)
  ...
  derivy_gen_bysem {d i J} :
    (∀ δ', self δ' → (⊢{δ d, i} →₁ ⊨{δ' ⊥ᵈ, i}) → (δ $∨ᵈ d →ᵈ δ' ⊥ᵈ) →
      (δ' ⊥ᵈ →ᵈ[≺ i] ⟦δ' ⊥ᵈ⟧ᵈ) → ⊨{δ' ⊥ᵈ, i} J) →
    ⊢{δ d, i} J
}.
...
Class inderivy (JU : judg) (δ' δ : pderiv_ty JU) (d : deriv_ty JU)
  ...
  (** Interpret [⊢{δ d, i}] as [⊨{δ' ⊥ᵈ, i}] *)
  inderivy_sem : ⊢{δ d, i} →₁ ⊨{δ' ⊥ᵈ, i};
  (** Turn [δ $∨ᵈ d] into [δ' ⊥ᵈ] *)
  inderivy_turn : δ $∨ᵈ d →ᵈ δ' ⊥ᵈ;
  (** Interpret [⊢{δ' ⊥ᵈ, j}] as [⊨{δ' ⊥ᵈ, j}] for [j ≺ i] *)
  inderivy_semlow : δ' ⊥ᵈ →ᵈ[≺ i] ⟦δ' ⊥ᵈ⟧ᵈ;
}.
...
Lemma derivy_bysem `{D : derivy JU δ} {d i J} :
  (∀ δ', inderivy JU δ' δ d i → ⊨{δ' ⊥ᵈ, i} J) → ⊢{δ d, i} J.
...
Lemma inderivy_turn_semlow `{!inderivy JU δ' δ d i} : δ $∨ᵈ d →ᵈ[≺ i] ⟦δ' ⊥ᵈ⟧ᵈ.
Proof. move=>/= *. apply inderivy_semlow; by [|apply inderivy_turn]. Qed.
Lemma inderivy_turn_semlow_l `{!inderivy JU δ' δ d i} :  δ d →ᵈ[≺ i] ⟦δ' ⊥ᵈ⟧ᵈ.
Proof. move=>/= *. apply inderivy_turn_semlow; by [|left]. Qed.
Lemma inderivy_turn_semlow_r `{!inderivy JU δ' δ d i} : d →ᵈ[≺ i] ⟦δ' ⊥ᵈ⟧ᵈ.
Proof. move=>/= *. apply inderivy_turn_semlow; by [|right]. Qed.

(** ** [deriv]: Derivability *)

(** Generator of [deriv] *)
Inductive deriv_gen (JU : judg) (self : deriv_ty JU) i
  (J : JU.(judg_data) i) : Prop := {
  (* Parameterization by [deriv_i] serves for strict positivity *)
  deriv_gen_bysem : ∃ deriv_i, (deriv_i →₁ deriv_gen JU self i) ∧
    (∀ δ', derivy JU δ' → (deriv_i →₁ ⊨{δ' ⊥ᵈ, i}) → (self →ᵈ δ' ⊥ᵈ) →
      (δ' ⊥ᵈ →ᵈ[≺ i] ⟦δ' ⊥ᵈ⟧ᵈ) → ⊨{δ' ⊥ᵈ, i} J)
}.
...
Record deriv_aty (JU : judg) := DerivAty {
  ...
  deriv_aty_data : JU.(judg_data) deriv_aty_idx;
}.
...
Definition deriv_gen' JU (d : deriv_aty JU → Prop) : deriv_aty JU → Prop :=
  deriv_uncurry (Dwrap (deriv_gen JU (deriv_curry d))).
...
(** Soundness of [deriv] *)
Lemma deriv_sound {JU} : deriv JU ⊥ᵈ →ᵈ ⟦deriv JU ⊥ᵈ⟧ᵈ.
Proof.
