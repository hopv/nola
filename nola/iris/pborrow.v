...
Section pborrow.
  ...
  #[export] Instance pbor_tok_timeless `{!OfeDiscrete (PROP $oi Σ)}
    {α X x ξ Φ} : Timeless (pbor_tok (X:=X) α x ξ Φ).
  ...
  Fact plend_tok_timeless `{!OfeDiscrete (PROP $oi Σ)} {α X xπ Φ} :
  ...
  Definition plend_body intp {X} (xπ : clair TY X) (Φ : X → PROP $oi Σ)
    : iProp Σ := ∃ x', ⟨π, xπ π = x'⟩ ∗ intp (Φ x').
  Definition plend_body_var intp {X} (ξ : prvar X) (Φ : X → PROP $oi Σ)
  ...
  Proof.
    iIntros "Pl →Ql".
    iMod (bor_lend_tok_new_list (M:=M) (intp:=pbintp _) α
  ...
  Proof.
    iIntros "l →Ql".
    iMod (lend_tok_split (M:=M) (intp:=pbintp _) ((λ Q, xjust Q) <$> Ql)
  ...
  Proof.
    iIntros "ol Ql →Pl".
    iMod (obor_tok_merge_subdiv (M:=M) (intp:=pbintp _)
  ...
  Lemma pbor_plend_tok_new_list α Xl (xΦl : plist _ Xl) :
  ...
  Lemma pbor_plend_tok_new α X x Φ :
  ...
  Lemma plend_tok_split {α X xπ Φ} Yl (yπΨl : plist (λ Y, _ *' (Y → _)) Yl) :
  ...
  Proof.
    iIntros "/=l →Ψl".
    iMod (lend_tok_split (M:=M) (intp:=pbintp _)
      (plend_list yπΨl) with "l [→Ψl]"); by rewrite big_sepL_of_plist.
  Qed.

  (** Retrieve from a prophetic lender *)
  Lemma plend_tok_retrieve {α X xπ Φ} :
    [†α] -∗ plend_tok (X:=X) α xπ Φ -∗ modw M (pborrow_wsat M intp)
      (plend_body intp xπ Φ).
  Proof. exact: lend_tok_retrieve. Qed.

  (** Open a prophetic borrower *)
  Lemma pborc_tok_open {α q X x ξ Φ} :
  ...
  Proof.
    ...
    iMod (obor_tok_merge_subdiv (M:=M) (intp:=pbintp _) _
  ...
  Proof.
    ...
    iMod (obor_tok_subdiv (M:=M) (intp:=pbintp _) [xpbor _ _ _ Ψ]
  ...
  Proof.
    ...
    iMod (vo_pc_update with "vo pc") as "[vo pc]".
    iMod (vo_pc_update with "vo' pc'") as "[vo' pc']".
    iMod (obor_tok_reborrow (M:=M) (intp:=pbintp _) α with "o' [pc' Ψ]")
      as "[β[c →b']]". { iExists _. iFrame. }
    rewrite lft_meet_comm. iMod vo_vo_alloc as (γx) "[vox vox']".
    iMod (obor_tok_subdiv (M:=M) (intp:=pbintp _) [xpreborrow _ _ γ γ' γx ξ f]
    ...
    iMod (obor_tok_merge_subdiv (M:=M) (intp:=pbintp _) [(_,_,_)';(_,_,_)']
      [xpbor _ γ'' η' Ψ]
