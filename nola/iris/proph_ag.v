...
Section proph_ag.
  ...
  #[export] Instance val_obs_timeless {γ X x} : Timeless (val_obs (X:=X) γ x).
  ...
  Local Lemma vo_vo2 {γ X x x'} :
    val_obs_def (X:=X) γ x -∗ val_obs2 (X:=X) γ x' -∗ False.
  Proof.
  ...
  Lemma vo_pc_proph_tok {γ X x x' ξ} :
    val_obs (X:=X) γ x -∗ proph_ctrl γ x' ξ -∗
      1:[ξ] ∗ (1:[ξ] -∗ val_obs γ x ∗ proph_ctrl γ x ξ).
  Proof.
    rewrite val_obs_unseal proph_ctrl_unseal.
